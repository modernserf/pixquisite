...
const middlewareHelper = (tests, unformat) => (options = {}) => {
    const { checkPayloads, onError } = { ...defaultMiddlewareOptions, ...options }
    return () => (next) => (action) => {
...
export default function makeSchema (schema, params = {}) {
    const { format, unformat, namespace } = { ...defaultParams, ...params }
